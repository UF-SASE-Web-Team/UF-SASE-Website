    ...
    <div className={cn("flex flex-col items-center gap-6")}>
      {Missions.map((mission) => (
        <div className={cn("bg-muted-background relative w-11/12 max-w-md rounded-lg p-5 shadow-lg")}>
          ...
          <div
            className={cn(
              { "border-saseGreen": mission.shadow == "green", "border-saseBlue": mission.shadow == "blue" },
              "absolute -left-2 -top-2 h-7 w-7 border-l-2 border-t-2",
            )}
          ></div>
          <div
            className={cn(
              { "border-saseGreen": mission.shadow == "green", "border-saseBlue": mission.shadow == "blue" },
              "absolute -bottom-2 -right-2 h-7 w-7 border-b-2 border-r-2",
            )}
          ></div>
        </div>
      ))}
